...
export class DashboardComponent implements OnInit {
  ...
  matchParticipationChart$: Observable<any> | undefined;
  matches$: Observable<Match[]> | undefined;
  nextMatch$: Observable<Match | undefined> | undefined;
  points$:
    | Observable<{ scoredPoints: number; collectedPoints: number }>
    | undefined;
  participations$: Observable<any> | undefined;
  attendance$: Observable<any> | undefined;
  strengths$: Observable<Tag[]> | undefined;
  improvements$: Observable<Tag[]> | undefined;
  weaknesses$: Observable<Tag[]> | undefined;
  ...
  ngOnInit(): void {
    ...
    this.season$
      ...
      .subscribe((season) => {
        const { teamId, id } = season as Season;
        this.store.dispatch(SeasonActions.loadSeasonById({ id }));
        this.store.dispatch(MatchActions.loadMatches({ teamId, seasonId: id }));
        this.store.dispatch(
          PlayerActions.loadPlayers({ teamId, seasonId: id })
        );
      });

    this.matchParticipationChart$ = this.matches$.pipe(
      map((matches) => {
        const pending = matches?.filter(
          (match) => match.status === 'Ausstehend'
        ).length;
        const won = matches?.filter((match) => match.status === 'Gewonnen')
          .length;
        const lost = matches?.filter((match) => match.status === 'Verloren')
          .length;
        return [
          ['Ausstehend', pending],
          ['Gewonnen', won],
    ...
    this.participations$ = this.matches$.pipe(
        ...
        matches.forEach((match) => {
          match.playerParticipations.forEach((playerParticipation) => {
            ...
            result[name] = {
              name: name,
              percentage:
                playerParticipation.percentage +
                (result[name]?.percentage ?? 0),
            };
          });
        });
    ...
    this.attendance$ = this.matches$.pipe(
      filter((matches) => !!matches),
      map((matches) => {
        let result: any = {};
        let data: any[] = [];
        ...
        matches.forEach((match) => {
          match.playerParticipations.forEach((playerParticipation) => {
            ...
            result[name] = {
              name: name,
              attendance: playerParticipation.didParticipate
                ? (result[name]?.attendance ?? 0) + 1
                : (result[name]?.attendance ?? 0) + 0,
            };
          });
        });
    ...
    this.strengths$ = this.players$.pipe(
      filter((players) => !!players?.length),
      map((players) => {
        let strengths = players?.map((player) => player.strengths) ?? [];
        let combined = strengths?.reduce((prev, current) => [
          ...prev,
          ...current,
        ]);
        return combined.sort((a, b) => a.name?.localeCompare(b?.name));
    ...
    this.improvements$ = this.players$.pipe(
      filter((players) => !!players?.length),
      map((players) => {
        let improvements = players?.map((player) => player.improvements) ?? [];
        let combined = improvements?.reduce((prev, current) => [
          ...prev,
          ...current,
        ]);
        return combined.sort((a, b) => a.name?.localeCompare(b?.name));
    ...
    this.nextMatch$ = this.matches$.pipe(
      map((matches) => {
        ...
        matches.forEach((match) => {
          const offset =
            new Date(match.date.seconds * 1000).valueOf() - today.valueOf();
